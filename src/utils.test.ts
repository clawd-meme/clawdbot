...
describe("resolveConfigDir", () => {
  it("prefers ~/.clawdbot when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "clawdbot-config-dir-"));
    try {
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/clawdbot")).toBe(path.resolve(os.homedir(), "clawdbot"));
  });
