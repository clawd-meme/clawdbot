...
function buildSandboxBrowserResolvedConfig(params: {
  ...
  return {
    ...
    color: DEFAULT_CLAWDBOT_BROWSER_COLOR,
    executablePath: undefined,
    headless: params.headless,
    noSandbox: false,
    attachOnly: true,
    defaultProfile: DEFAULT_CLAWDBOT_BROWSER_PROFILE_NAME,
    profiles: {
      [DEFAULT_CLAWDBOT_BROWSER_PROFILE_NAME]: {
...
export async function ensureSandboxBrowser(params: {
  ...
  const existingProfile = existing
    ? resolveProfile(existing.bridge.state.resolved, DEFAULT_CLAWDBOT_BROWSER_PROFILE_NAME)
    : null;
  const shouldReuse =
