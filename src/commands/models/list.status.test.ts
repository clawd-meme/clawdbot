...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/clawdbot-agent"),
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveClawdbotAgentDir).toHaveBeenCalled();
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/clawdbot-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/clawdbot-agent-custom");
