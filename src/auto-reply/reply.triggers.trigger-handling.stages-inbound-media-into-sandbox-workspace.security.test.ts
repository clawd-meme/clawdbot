...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(async (home) => await fn(home), { prefix: "clawdbot-triggers-bypass-" });
}
...
describe("stageSandboxMedia security", () => {
  it("rejects staging host files from outside the media directory", async () => {
    await withTempHome(async (home) => {
      // Sensitive host file outside .clawdbot
      ...
      await stageSandboxMedia({
        ...
        workspaceDir: join(home, "clawdbot"),
      });
