...
import {
  ...
  DEFAULT_CLAWDBOT_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_CLAWDBOT_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "clawdbot" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_CLAWDBOT_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_CLAWDBOT_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Clawdbot browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for clawdbot/clawdbot CDP).
...
export function resolveBrowserConfig(
  ...
  const enabled = cfg?.enabled ?? DEFAULT_CLAWDBOT_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "clawdbot";
