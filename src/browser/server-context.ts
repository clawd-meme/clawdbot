...
import {
  isChromeCdpReady,
  isChromeReachable,
  launchClawdbotChrome,
  resolveClawdbotUserDataDir,
  stopClawdbotChrome,
} from "./chrome.js";
...
function createProfileContext(
  ...
  const ensureBrowserAvailable = async (): Promise<void> => {
    ...
    if (!httpReachable) {
      ...
      const launched = await launchClawdbotChrome(current.resolved, profile);
    ...
    await stopClawdbotChrome(profileState.running);
    setProfileRunning(null);

    const relaunched = await launchClawdbotChrome(current.resolved, profile);
