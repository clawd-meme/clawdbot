...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  ...
  if (looksLikeTimeout) {
    return new Error(
      `Can't reach the clawdbot browser control service (timed out after ${timeoutMs}ms). ${hint}`,
    );
  }
  return new Error(`Can't reach the clawdbot browser control service. ${hint} (${msg})`);
}
