...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".clawdbot-profile-decorated");
}
 ...
 */
export function decorateClawdbotProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_CLAWDBOT_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_CLAWDBOT_BROWSER_COLOR).toUpperCase();
