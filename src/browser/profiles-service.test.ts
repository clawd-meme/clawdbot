...
vi.mock("./chrome.js", () => ({
  resolveClawdbotUserDataDir: vi.fn(() => "/tmp/clawdbot-test/clawdbot/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveClawdbotUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "clawdbot-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveClawdbotUserDataDir).mockReturnValue(userDataDir);
