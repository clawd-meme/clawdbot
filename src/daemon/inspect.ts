...
export type ExtraGatewayService = {
  ...
  detail: string;
  scope: "user" | "system";
  marker?: "clawdbot" | "clawdbot" | "moltbot";
  legacy?: boolean;
};
...
const EXTRA_MARKERS = ["clawdbot", "clawdbot", "moltbot"] as const;
...
function hasGatewayServiceMarker(content: string): boolean {
  const lower = content.toLowerCase();
  const markerKeys = ["clawdbot_service_marker"];
  const kindKeys = ["clawdbot_service_kind"];
...
function isClawdbotGatewayLaunchdService(label: string, contents: string): boolean {
  ...
  return label.startsWith("ai.clawdbot.");
}
...
async function scanLaunchdDir(params: {
  ...
  for (const entry of entries) {
    ...
    if (marker === "clawdbot" && isClawdbotGatewayLaunchdService(label, contents)) {
      continue;
    }
    results.push({
      platform: "darwin",
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "clawdbot" || isLegacyLabel(label),
    });
  }
...
async function scanSystemdDir(params: {
  ...
  for (const entry of entries) {
    ...
    if (marker === "clawdbot" && isClawdbotGatewaySystemdService(name, contents)) {
      continue;
    }
    results.push({
      platform: "linux",
