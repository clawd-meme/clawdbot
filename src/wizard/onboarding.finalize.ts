...
export async function finalizeOnboardingWizard(options: FinalizeOnboardingOptions) {
  ...
  if (!opts.skipHealth) {
    ...
    try {
      ...
      await prompter.note(
        [
          "Docs:",
          "https://docs.clawdbot.ai/gateway/health",
          "https://docs.clawdbot.ai/gateway/troubleshooting",
        ].join("\n"),
        "Health check help",
      );
    }
  ...
  await prompter.note(
    [
      ...
      gatewayStatusLine,
      "Docs: https://docs.clawdbot.ai/web/control-ui",
    ]
      .filter(Boolean)
      .join("\n"),
    "Control UI",
  ...
  await prompter.note(
    [
      ...
      "Docs: https://docs.clawdbot.ai/concepts/agent-workspace",
    ].join("\n"),
    "Workspace backup",
  );

  await prompter.note(
    "Running agents on your computer is risky — harden your setup: https://docs.clawdbot.ai/security",
    "Security",
  );
  ...
  await prompter.note(
    hasWebSearchKey
      ? [
          ...
          "Docs: https://docs.clawdbot.ai/tools/web",
        ].join("\n")
      : [
  ...
  await prompter.note(
    'What now: https://clawdbot.ai/showcase ("What People Are Building").',
    "What now",
  );
