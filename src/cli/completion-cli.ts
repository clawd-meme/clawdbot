...
export async function installCompletion(shell: string, yes: boolean, binName = "clawdbot") {
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c clawdbot -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'clawdbot gateway' and not 'clawdbot other gateway', we need to check the sequence.
