...
const DEFAULT_PACKAGE_NAME = "clawdbot";
...
function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("clawdbot@")) {
    return trimmed.slice("clawdbot@".length);
  }
...
async function ensureGitCheckout(params: {
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `CLAWDBOT_GIT_DIR points at a non-git directory: ${params.dir}. Set CLAWDBOT_GIT_DIR to an empty folder or an clawdbot checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `CLAWDBOT_GIT_DIR points at a non-git directory: ${gitDir}. Set CLAWDBOT_GIT_DIR to an empty folder or an clawdbot checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.clawdbot.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ...
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.clawdbot.ai/cli/update")}`,
    )
    .action(async (opts) => {
      try {
