...
describe("control UI assets helpers", () => {
  ...
  it("resolves dist control-ui index path from package root argv1", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "clawdbot-ui-"));
    try {
      await fs.writeFile(path.join(tmp, "package.json"), JSON.stringify({ name: "clawdbot" }));
      await fs.writeFile(path.join(tmp, "clawdbot.mjs"), "export {};\n");
      ...
      expect(await resolveControlUiDistIndexPath(path.join(tmp, "clawdbot.mjs"))).toBe(
  ...
  it("resolves dist control-ui index path from .bin argv1", async () => {
    ...
    try {
      ...
      const pkgRoot = path.join(tmp, "node_modules", "clawdbot");
      await fs.mkdir(binDir, { recursive: true });
      await fs.mkdir(path.join(pkgRoot, "dist", "control-ui"), { recursive: true });
      await fs.writeFile(path.join(binDir, "clawdbot"), "#!/usr/bin/env node\n");
      await fs.writeFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "clawdbot" }));
      await fs.writeFile(path.join(pkgRoot, "dist", "control-ui", "index.html"), "<html></html>\n");

      expect(await resolveControlUiDistIndexPath(path.join(binDir, "clawdbot"))).toBe(
