...
describe("gateway bonjour advertiser", () => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    process.env.CLAWDBOT_MDNS_HOSTNAME = "test-host";
    process.env.CLAWDBOT_MDNS_HOSTNAME = "test-host";
  ...
  it("normalizes hostnames with domains for service names", async () => {
    ...
    expect(gatewayCall?.[0]?.name).toBe("clawdbot (Clawdbot)");
    expect(gatewayCall?.[0]?.domain).toBe("local");
    expect(gatewayCall?.[0]?.hostname).toBe("clawdbot");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("clawdbot.local");
