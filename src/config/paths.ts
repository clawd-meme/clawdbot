...
const NEW_STATE_DIRNAME = ".clawdbot";
const CONFIG_FILENAME = "clawdbot.json";
 ...
 * Can be overridden via CLAWDBOT_STATE_DIR.
 * Default: ~/.clawdbot
 */
export function resolveStateDir(
  ...
  const override = env.CLAWDBOT_STATE_DIR?.trim() || env.CLAWDBOT_STATE_DIR?.trim();
  if (override) {
 ...
 * Can be overridden via CLAWDBOT_CONFIG_PATH.
...
export function resolveCanonicalConfigPath(
  ...
  const override = env.CLAWDBOT_CONFIG_PATH?.trim() || env.CLAWDBOT_CONFIG_PATH?.trim();
  if (override) {
    return resolveUserPath(override);
  }
...
export function resolveDefaultConfigCandidates(
  ...
  const explicit = env.CLAWDBOT_CONFIG_PATH?.trim() || env.CLAWDBOT_CONFIG_PATH?.trim();
  if (explicit) {
    return [resolveUserPath(explicit)];
  }

  const candidates: string[] = [];
  const clawdbotStateDir = env.CLAWDBOT_STATE_DIR?.trim() || env.CLAWDBOT_STATE_DIR?.trim();
  if (clawdbotStateDir) {
    const resolved = resolveUserPath(clawdbotStateDir);
...
export function resolveGatewayPort(
  ...
  const envRaw = env.CLAWDBOT_GATEWAY_PORT?.trim() || env.CLAWDBOT_GATEWAY_PORT?.trim();
  if (envRaw) {
