...
import type { DiagnosticEventPayload, ClawdbotPluginService } from "clawdbot/plugin-sdk";
    ...
    async start(ctx) {
      ...
      if (logsEnabled) {
        ...
        stopLogTransport = registerLogTransport((logObj) => {
          ...
          if (meta?.name) {
            attributes["clawdbot.logger"] = meta.name;
          }
      ...
      const recordModelUsage = (evt: Extract<DiagnosticEventPayload, { type: "model.usage" }>) => {
        ...
        if (usage.input) {
          tokensCounter.add(usage.input, { ...attrs, "clawdbot.token": "input" });
        }
        if (usage.output) {
      ...
      const recordWebhookProcessed = (
        ...
        if (evt.chatId !== undefined) {
          spanAttrs["clawdbot.chatId"] = String(evt.chatId);
        }
      ...
      const recordMessageProcessed = (
        ...
        const spanAttrs: Record<string, string | number> = { ...attrs };
        if (evt.sessionKey) {
          spanAttrs["clawdbot.sessionKey"] = evt.sessionKey;
        }
        if (evt.sessionId) {
          spanAttrs["clawdbot.sessionId"] = evt.sessionId;
        }
        if (evt.chatId !== undefined) {
          spanAttrs["clawdbot.chatId"] = String(evt.chatId);
        }
        if (evt.messageId !== undefined) {
          spanAttrs["clawdbot.messageId"] = String(evt.messageId);
        }
        if (evt.reason) {
          spanAttrs["clawdbot.reason"] = evt.reason;
        }
      ...
      const recordSessionState = (
        ...
        if (evt.reason) {
          attrs["clawdbot.reason"] = evt.reason;
        }
