...
- This plugin integrates Lobster with Clawdbot _without core changes_.
...
## Using `clawdbot.invoke` (Lobster → Clawdbot tools)

Some Lobster pipelines may include a `clawdbot.invoke` step to call back into Clawdbot tools/plugins (for example: `gog` for Google Workspace, `gh` for GitHub, `message.send`, etc.).
...
To avoid letting workflows call arbitrary tools, set a tight allowlist on the agent that will be used by `clawdbot.invoke`.
