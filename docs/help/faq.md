...
- [Quick start and first-run setup](#quick-start-and-firstrun-setup)
  ...
  - [I can't access docs.clawdbot.ai (SSL error). What now?](#i-cant-access-docsclawdbotai-ssl-error-what-now)
...
- [How do I install the beta version, and what’s the difference between beta and dev?](#how-do-i-install-the-beta-version-and-whats-the-difference-between-beta-and-dev)
  ...
  - [Can I ask Clawdbot to update itself?](#can-i-ask-clawdbot-to-update-itself)
  ...
  - [Can multiple people use one WhatsApp number with different Clawdbot instances?](#can-multiple-people-use-one-whatsapp-number-with-different-clawdbot-instances)
...
- [Remote gateways + nodes](#remote-gateways-nodes)
  ...
  - [Can two Clawdbot instances talk to each other (local + VPS)?](#can-two-clawdbot-instances-talk-to-each-other-local-vps)
...
- [Sessions & multiple chats](#sessions-multiple-chats)
  ...
  - [Is there a way to make a team of Clawdbot instances one CEO and many agents](#is-there-a-way-to-make-a-team-of-clawdbot-instances-one-ceo-and-many-agents)
...
- [Models: defaults, selection, aliases, switching](#models-defaults-selection-aliases-switching)
  ...
  - [What do Clawdbot, Flawd, and Krill use for models?](#what-do-clawdbot-flawd-and-krill-use-for-models)
...
```bash
curl -fsSL https://clawdbot.ai/install.sh | bash -s -- --install-method git
```
...
3. Copy your workspace (default: `~/.clawdbot/workspace`).
...
### I cant access docs.clawdbot.ai SSL error What now

Some Comcast/Xfinity connections incorrectly block `docs.clawdbot.ai` via Xfinity
Advanced Security. Disable it or allowlist `docs.clawdbot.ai`, then retry. More
detail: [Troubleshooting](/help/troubleshooting#docsclawdbotai-shows-an-ssl-error-comcastxfinity).
...
```bash
curl -fsSL --proto '=https' --tlsv1.2 https://clawdbot.ai/install.sh | bash -s -- --beta
```

```bash
curl -fsSL --proto '=https' --tlsv1.2 https://clawdbot.ai/install.sh | bash -s -- --install-method git
```

Windows installer (PowerShell):
https://clawdbot.ai/install.ps1
...
```bash
curl -fsSL https://clawdbot.ai/install.sh | bash -s -- --verbose
```

Beta install with verbose:

```bash
curl -fsSL https://clawdbot.ai/install.sh | bash -s -- --beta --verbose
```

For a hackable (git) install:

```bash
curl -fsSL https://clawdbot.ai/install.sh | bash -s -- --install-method git --verbose
```
...
### Can I ask Clawdbot to update itself
...
### Can multiple people use one WhatsApp number with different Clawdbot instances
...
(`~/.clawdbot`) and workspace (`~/.clawdbot/workspace`) stay untouched.
...
Showcase: https://clawdbot.ai/showcase
...
Yes. Add extra directories via `skills.load.extraDirs` in `~/.clawdbot/clawdbot.json` (lowest precedence). Default precedence remains: `<workspace>/skills` → `~/.clawdbot/skills` → bundled → `skills.load.extraDirs`. `clawhub` installs into `./skills` by default, which Clawdbot treats as `<workspace>/skills`.
...
ClawHub installs into `./skills` under your current directory (or falls back to your configured Clawdbot workspace); Clawdbot treats that as `<workspace>/skills` on the next session. For shared skills across agents, place them in `~/.clawdbot/skills/<name>/SKILL.md`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills) and [ClawHub](/tools/clawhub).
...
Your **workspace** (AGENTS.md, memory files, skills, etc.) is separate and configured via `agents.defaults.workspace` (default: `~/.clawdbot/workspace`).
...
Default workspace is `~/.clawdbot/workspace`, configurable via:

```json5
{
  agents: { defaults: { workspace: "~/.clawdbot/workspace" } },
}
```
...
If the file is missing, it uses safe‑ish defaults (including a default workspace of `~/.clawdbot/workspace`).
...
### Can two Clawdbot instances talk to each other local VPS
...
### Is there a way to make a team of Clawdbot instances one CEO and many agents
...
### What do Clawdbot, Flawd, and Krill use for models

- **Clawdbot + Flawd:** Anthropic Opus (`anthropic/claude-opus-4-5`) - see [Anthropic](/providers/anthropic).
...
Profiles also suffix service names (`bot.molt.<profile>`; legacy `com.clawdbot.*`, `clawdbot-gateway-<profile>.service`, `Clawdbot Gateway (<profile>)`).
...
Outbound attachments from the agent must include a `MEDIA:<path-or-url>` line (on its own line). See [Clawdbot assistant setup](/start/clawdbot) and [Agent send](/tools/agent-send).
