...
- [ ] Confirm package metadata (name, description, repository, keywords, license) and `bin` map points to [`clawdbot.mjs`](https://github.com/clawdbot/clawdbot/blob/main/clawdbot.mjs) for `clawdbot`.
...
- [ ] (Optional) Installer E2E (Docker, runs `curl -fsSL https://clawdbot.ai/install.sh | bash`, onboards, then runs real tool calls):
