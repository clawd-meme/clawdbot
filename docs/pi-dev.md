...
This guide summarizes a sane workflow for working on the pi integration in Clawdbot.
...
- Trigger the agent directly:
  - `pnpm clawdbot agent --message "Hello" --thinking low`
...
State lives under the Clawdbot state directory. Default is `~/.clawdbot`. If `CLAWDBOT_STATE_DIR` is set, use that directory instead.

To reset everything:

- `clawdbot.json` for config
...
- https://docs.clawdbot.ai/testing
- https://docs.clawdbot.ai/start/getting-started
