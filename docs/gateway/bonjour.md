...
Clawdbot supports any discovery domain; `clawdbot.internal.` is just an example.
...
- serve your chosen domain (example: `clawdbot.internal.`) from `~/.clawdbot/dns/<domain>.db`
...
`_clawdbot-gw._tcp` in your discovery domain without multicast.
...
- `CLAWDBOT_DISABLE_BONJOUR=1` disables advertising (legacy: `CLAWDBOT_DISABLE_BONJOUR`).
- `gateway.bind` in `~/.clawdbot/clawdbot.json` controls the Gateway bind mode.
- `CLAWDBOT_SSH_PORT` overrides the SSH port advertised in TXT (legacy: `CLAWDBOT_SSH_PORT`).
- `CLAWDBOT_TAILNET_DNS` publishes a MagicDNS hint in TXT (legacy: `CLAWDBOT_TAILNET_DNS`).
- `CLAWDBOT_CLI_PATH` overrides the advertised CLI path (legacy: `CLAWDBOT_CLI_PATH`).
