...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@clawdbot", "@mybot"] }
      }
    ]
  }
}
...
- Prefer a dedicated profile for the agent (the default `clawdbot` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only Clawdbot clients can connect.
