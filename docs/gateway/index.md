...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__clawdbot__/canvas/` from `~/.clawdbot/workspace/canvas`. Disable with `canvasHost.enabled=false` or `CLAWDBOT_SKIP_CANVAS_HOST=1`.
...
- `agents.defaults.workspace` default becomes `~/.clawdbot/workspace-dev` when you run `setup`/`onboard` under `--dev`.
...
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `clawdbot gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
...
- To stop it cleanly, use `clawdbot gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `clawdbot gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
