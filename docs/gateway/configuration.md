...
If the file is missing, Clawdbot uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.clawdbot/workspace`). You usually only need a config to:
...
{
  agents: {
    defaults: { workspace: "~/.clawdbot/workspace" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@clawdbot", "reisponde"] },
      },
    ],
  },
  channels: {
...
// ~/.clawdbot/agents.json5
{
  ...
  list: [{ id: "main", workspace: "~/.clawdbot/workspace" }],
}
```
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@clawdbot", "clawdbot"] } }],
  },
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@clawdbot"],
        },
      },
    ],
  },
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.clawdbot/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.clawdbot/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.clawdbot/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.clawdbot/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.clawdbot/workspace-home" },
      { id: "work", workspace: "~/.clawdbot/workspace-work" },
    ],
  },
  bindings: [
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["clawdbot-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
          // guild id (preferred) or slug
          slug: "friends-of-clawdbot",
...
- Set `KIMI_API_KEY` in the environment or use `clawdbot onboard --auth-choice kimi-code-api-key`.
...
### `browser` (clawdbot-managed browser)

Clawdbot can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for clawdbot and expose a small loopback control service.
...
Default root: `~/.clawdbot/workspace/canvas`  
Default port: `18793` (chosen to avoid the clawdbot browser CDP port `18792`)  
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__clawdbot__/ws`
...
{
  canvasHost: {
    root: "~/.clawdbot/workspace/canvas",
    port: 18793,
    liveReload: true,
  },
}
...
- Hostname: defaults to `clawdbot` (advertises `clawdbot.local`). Override with `CLAWDBOT_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_clawdbot-gw._tcp` under `~/.clawdbot/dns/` using the configured discovery domain (example: `clawdbot.internal.`).
