...
- **Tailoring lives outside the repo:** `~/.clawdbot/workspace` (workspace) + `~/.clawdbot/clawdbot.json` (config).
...
- **Workspace:** `~/.clawdbot/workspace` (skills, prompts, memories; make it a private git repo)
...
- Keep `~/.clawdbot/workspace` and `~/.clawdbot/` as “your stuff”; don’t put personal prompts/config into the `clawdbot` repo.
...
- [Clawdbot assistant setup](/start/clawdbot)
