...
```bash
curl -fsSL https://clawdbot.ai/install.sh | bash -s -- --no-onboard
```
...
pnpm add -g clawdbot@latest           # re-run to execute postinstall scripts
```
...
```bash
# Explicit npm
curl -fsSL https://clawdbot.ai/install.sh | bash -s -- --install-method npm
