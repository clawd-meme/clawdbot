...
read_when:
  - You want to understand `clawdbot.ai/install.sh`
...
Clawdbot ships two installer scripts (served from `clawdbot.ai`):

- `https://clawdbot.ai/install.sh` — “recommended” installer (global npm install by default; can also install from a GitHub checkout)
- `https://clawdbot.ai/install-cli.sh` — non-root-friendly CLI installer (installs into a prefix with its own Node)
- `https://clawdbot.ai/install.ps1` — Windows PowerShell installer (npm by default; optional git install)

To see the current flags/behavior, run:

```bash
curl -fsSL https://clawdbot.ai/install.sh | bash -s -- --help
```

Windows (PowerShell) help:

```powershell
& ([scriptblock]::Create((iwr -useb https://clawdbot.ai/install.ps1))) -?
```
...
```bash
SHARP_IGNORE_GLOBAL_LIBVIPS=0 curl -fsSL https://clawdbot.ai/install.sh | bash
```
...
```bash
curl -fsSL https://clawdbot.ai/install-cli.sh | bash -s -- --help
```
...
```powershell
iwr -useb https://clawdbot.ai/install.ps1 | iex
```

```powershell
iwr -useb https://clawdbot.ai/install.ps1 | iex -InstallMethod git
```

```powershell
iwr -useb https://clawdbot.ai/install.ps1 | iex -InstallMethod git -GitDir "C:\\clawdbot"
```
