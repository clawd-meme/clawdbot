...
- For **source installs**, use:
  ```bash
  curl -fsSL https://clawdbot.ai/install.sh | bash -s -- --install-method git --no-onboard
  ```
...
- `pnpm build` matters when you run the packaged `clawdbot` binary ([`clawdbot.mjs`](https://github.com/clawdbot/clawdbot/blob/main/clawdbot.mjs)) or use Node to run `dist/`.
