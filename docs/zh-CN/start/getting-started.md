...
```bash
curl -fsSL https://clawdbot.bot/install.sh | bash
```
...
如果您尚未进行全局安装，请通过以下方式运行上手引导步骤 `pnpm clawdbot ...` （从仓库中）。
`pnpm build` 也会打包 A2UI 资源；如果您只需要运行该步骤，请使用 `pnpm canvas:a2ui:bundle`。

Gateway（从此仓库）：

```bash
node clawdbot.mjs gateway --port 18789 --verbose
```
...
如果 `clawdbot health` 显示"未配置认证"，请返回向导设置 OAuth/密钥认证——智能体在没有认证的情况下将无法响应。

提示： `clawdbot status --all` 是最佳的可粘贴只读调试报告。
健康探针： `clawdbot health` （或 `clawdbot status --deep`）向运行中的 Gateway 请求健康快照。
