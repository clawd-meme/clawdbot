...
read_when:
  - Designing workspace memory (~/.clawdbot/workspace) beyond daily Markdown logs
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.clawdbot/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.clawdbot/workspace` as canonical human-readable memory.
...
```
~/.clawdbot/workspace/.memory/index.sqlite
```
...
Pragmatic take for `~/.clawdbot/workspace`:
