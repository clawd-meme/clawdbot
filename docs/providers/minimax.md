...
```bash
clawdbot plugins enable minimax-portal-auth  # skip if already loaded.
clawdbot gateway restart  # restart if gateway is already running
clawdbot onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/clawdbot/clawdbot/tree/main/extensions/minimax-portal-auth) for details.
