...
read_when:
  ...
  - Debugging why clawdbot is interfering with your own Chrome
  - Implementing browser settings + lifecycle in the macOS app
title: "Browser (Clawdbot-managed)"
---

# Browser (clawdbot-managed)
...
- The `clawdbot` profile does **not** touch your personal browser profile.
- The agent can **open tabs, read pages, click, and type** in a safe lane.
- The default `chrome` profile uses the **system default Chromium browser** via the
  extension relay; switch to `clawdbot` for the isolated managed browser.

## What you get

- A separate browser profile named **clawdbot** (orange accent by default).
- Deterministic tab control (list/open/focus/close).
- Agent actions (click/type/drag/select), snapshots, screenshots, PDFs.
- Optional multi-profile support (`clawdbot`, `work`, `remote`, ...).
...
```bash
clawdbot browser --browser-profile clawdbot status
clawdbot browser --browser-profile clawdbot start
clawdbot browser --browser-profile clawdbot open https://example.com
clawdbot browser --browser-profile clawdbot snapshot
```

If you get “Browser disabled”, enable it in config (see below) and restart the
Gateway.

## Profiles: `clawdbot` vs `chrome`

- `clawdbot`: managed, isolated browser (no extension required).
- `chrome`: extension relay to your **system browser** (requires the Clawdbot
  extension to be attached to a tab).

Set `browser.defaultProfile: "clawdbot"` if you want managed mode by default.
...
```json5
{
  browser: {
    ...
    profiles: {
      clawdbot: { cdpPort: 18800, color: "#FF4500" },
...
- Default profile is `chrome` (extension relay). Use `defaultProfile: "clawdbot"` for the managed browser.
- Auto-detect order: system default browser if Chromium-based; otherwise Chrome → Brave → Edge → Chromium → Chrome Canary.
- Local `clawdbot` profiles auto-assign `cdpPort`/`cdpUrl` — set those only for remote CDP.
...
- **clawdbot-managed**: a dedicated Chromium-based browser instance with its own user data directory + CDP port
- **remote**: an explicit CDP URL (Chromium-based browser running elsewhere)
- **extension relay**: your existing Chrome tab(s) via the local relay + Chrome extension

Defaults:

- The `clawdbot` profile is auto-created if missing.
...
Clawdbot can also drive **your existing Chrome tabs** (no separate “clawdbot” Chrome instance) via a local CDP relay + a Chrome extension.
...
error. ARIA snapshots and basic screenshots still work for clawdbot-managed Chrome.
...
- `snapshot`:
  ...
  - Config default (tool/CLI only): set `browser.snapshotDefaults.mode: "efficient"` to use efficient snapshots when the caller does not pass a mode (see [Gateway configuration](/gateway/configuration#browser-clawdbot-managed-browser)).
...
- The clawdbot browser profile may contain logged-in sessions; treat it as sensitive.
...
- `browser` accepts:
  - `profile` to choose a named browser profile (clawdbot, chrome, or remote CDP).
