...
gog.gmail.search --query 'newer_than:1d' \
  | clawdbot.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
```lobster
clawdbot.invoke --tool llm-task --action json --args-json '{
...
- **No secrets** — Lobster doesn't manage OAuth; it calls Clawdbot tools that do.
