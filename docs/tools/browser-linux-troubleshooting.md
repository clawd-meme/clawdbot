...
```
{"error":"Error: Failed to start Chrome CDP on port 18800 for profile \"clawdbot\"."}
```
...
chromium-browser --headless --no-sandbox --disable-gpu \
  ...
  --user-data-dir=$HOME/.clawdbot/browser/clawdbot/user-data \
  about:blank &
```
...
```ini
# ~/.config/systemd/user/clawdbot-browser.service
[Unit]
Description=Clawdbot Browser (Chrome CDP)
After=network.target

[Service]
ExecStart=/snap/bin/chromium --headless --no-sandbox --disable-gpu --remote-debugging-port=18800 --user-data-dir=%h/.clawdbot/browser/clawdbot/user-data about:blank
Restart=on-failure
RestartSec=5
...
Enable with: `systemctl --user enable --now clawdbot-browser.service`
...
1. **Use the managed browser:** `clawdbot browser start --browser-profile clawdbot`
   (or set `browser.defaultProfile: "clawdbot"`).
...
- Local `clawdbot` profiles auto-assign `cdpPort`/`cdpUrl`; only set those for remote CDP.
