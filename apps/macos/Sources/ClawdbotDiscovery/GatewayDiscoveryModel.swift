...
public final class GatewayDiscoveryModel {
    ...
    private let logger = Logger(subsystem: "ai.clawdbot", category: "gateway-discovery")
    ...
    public func start() {
        ...
        for domain in ClawdbotBonjour.gatewayServiceDomains {
            ...
            browser.start(queue: DispatchQueue(label: "ai.clawdbot.macos.gateway-discovery.\(domain)"))
        }
    ...
    public func refreshWideAreaFallbackNow(timeoutSeconds: TimeInterval = 5.0) {
        guard let domain = ClawdbotBonjour.wideAreaGatewayServiceDomain else { return }
    ...
    private func updateGateways(for domain: String) {
        ...
        if let wideAreaDomain = ClawdbotBonjour.wideAreaGatewayServiceDomain,
    ...
    private var hasUsableWideAreaResults: Bool {
        guard let domain = ClawdbotBonjour.wideAreaGatewayServiceDomain else { return false }
