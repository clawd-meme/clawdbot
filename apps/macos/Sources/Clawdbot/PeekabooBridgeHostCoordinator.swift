...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.clawdbot", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: ClawdbotPeekabooBridgeServices?
    private static var clawdbotSocketPath: String {
    ...
    private func startIfNeeded() async {
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.clawdbotSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.clawdbotSocketPath, privacy: .public)")
    }
...
private final class ClawdbotPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.clawdbot.peekaboo")
