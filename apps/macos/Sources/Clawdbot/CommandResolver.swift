...
enum CommandResolver {
    ...
    static func gatewayEntrypoint(in root: URL) -> String? {
        ...
        let clawdbotEntry = root.appendingPathComponent("clawdbot.mjs").path
        if FileManager().isReadableFile(atPath: clawdbotEntry) { return clawdbotEntry }
    ...
    private static func clawdbotManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".clawdbot"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("clawdbot.mjs").path,
            root.appendingPathComponent("bin/clawdbot.js").path,
        ]
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            ...
            let missingEntry = """
            clawdbot entrypoint missing (looked for dist/index.js or clawdbot.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/clawdbot.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/clawdbot.mjs"
            node "$PRJ/clawdbot.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
