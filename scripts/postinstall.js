...
function trySetupCompletion(repoRoot) {
  ...
  if (process.env.CI || process.env.CLAWDBOT_SKIP_COMPLETION_SETUP) {
    return;
  }

  const binPath = path.join(repoRoot, "clawdbot.mjs");
  ...
  try {
    // Run with CLAWDBOT_SKIP_POSTINSTALL to avoid any weird recursion,
    // though distinct from this script.
    spawnSync(process.execPath, [binPath, "completion", "--install", "--yes"], {
      cwd: repoRoot,
      stdio: "inherit",
      env: { ...process.env, CLAWDBOT_SKIP_POSTINSTALL: "1" },
    });
  } catch {
