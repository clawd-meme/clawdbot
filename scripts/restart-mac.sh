...
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.clawdbot.mac.plist"
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log "  node clawdbot.mjs daemon install --force --runtime node"
      log "  node clawdbot.mjs daemon restart"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.clawdbot.mac 2>/dev/null || true
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node clawdbot.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node clawdbot.mjs daemon restart"
