...
SERVER="${CLAWDBOT_SERVER:-${CLAWDBOT_SERVER:-l36}}"
RESULT=$(ssh "$SERVER" '/home/admin/clawdbot/scripts/sync-claude-code-auth.sh' 2>&1)
EXIT_CODE=$?
...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    ssh "$SERVER" 'systemctl --user restart clawdbot' 2>/dev/null
else
